...
function EarningsPage() {
  ...
  const data = [
    {
      name: "Comissão de imóveis",
      value: comissao,
      color: "oklch(0.21 0.05 255)",
      pct: pctComissao,
    },
            ...
            <div>
              ...
              <div className="mt-2 space-y-0.5 text-xs text-white/60">
                <div className="flex justify-between">
                  ...
                  <span className="text-white/80">{ativos}</span>
                </div>
                <div className="flex justify-between">
                  <span>Recorrência mensal</span>
                  <span className="num text-white/80">{formatBRL(mrrTotal)}/mês</span>
                </div>
              </div>
            </div>
          </div>
              ...
              {faltaMensalidade > 0 ? (
                <>
                  ...
                  <span className="num font-medium text-foreground">
                    {formatBRL(faltaMensalidade)}
                  </span>{" "}
                  para cobrir sua mensalidade.
                </>
              ) : (
          ...
          <div className="mt-5 grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div className="rounded-xl border border-border bg-surface p-4">
              ...
              <div className="mt-1 text-xs text-emerald-700">
                projetado a partir do desempenho parcial
              </div>
            </div>
            <div className="rounded-xl border border-border bg-surface p-4">
              <div className="text-xs uppercase tracking-widest text-muted-foreground">
      ...
      <div className="rounded-2xl border border-border bg-card">
        ...
        {transactions.length === 0 ? (
          <p className="p-8 text-center text-sm text-muted-foreground">
            Nenhuma transação ainda. Registre uma venda em Imóveis ou convide corretores em
            Indicações.
          </p>
        ) : (
              ...
              {transactions.map((t) => (
                <tr key={t.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-5 py-3">
                    ...
                    >
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${t.tipo === "Comissão" ? "bg-blue-600" : "bg-orange-500"}`}
                      />
                      {t.tipo}
                    </span>
                  </td>
