...
const STATUS_LIST: StatusFiltro[] = [
  "Todos",
  "Pendente",
  "Faturado",
  "Pago",
...
function FinanceiroPage() {
  ...
  const [conciliacoes, setConciliacoes] = useState<Conciliacao[]>(() =>
    conciliacoesMock.map((c) => ({
      ...c,
      status: calcularStatusConciliacao(c.esperado, c.recebido),
    })),
  );
  ...
  const [concRiscoFiltro, setConcRiscoFiltro] = useState<"Todos" | "baixo" | "medio" | "alto">(
    "Todos",
  );
  ...
  function atualizarConciliacao(
    id: string,
    patch: Partial<Conciliacao>,
    audit?: ConciliacaoAuditoria,
  ) {
  ...
  function reabrirConciliacao(c: Conciliacao, justificativa: string) {
    ...
    atualizarConciliacao(
      c.id,
      { recebido: 0, pagoEm: undefined, statusOperacional: "Em cobrança" },
      {
        data: agora(),
        autor: "Superadmin",
        acao: `Conciliação reaberta — Justificativa: ${justificativa}`,
        valorAnterior: c.recebido,
        valorNovo: 0,
      },
    );
  }
  ...
  function adicionarInteracao(id: string, interacao: ConciliacaoInteracao) {
    setConciliacoes((prev) =>
      prev.map((c) => (c.id === id ? { ...c, interacoes: [...c.interacoes, interacao] } : c)),
    );
    setConcDetalhe((cur) =>
      cur && cur.id === id ? { ...cur, interacoes: [...cur.interacoes, interacao] } : cur,
    );
  }
  ...
  const concFiltradas = useMemo(() => {
    const lista = conciliacoes.filter((c) => {
      ...
      const min = parseFloat(concValMin);
      const max = parseFloat(concValMax);
  ...
  const concKpis = useMemo(() => {
    const conciliado = concFiltradas
      .filter((c) => c.status === "Confirmada")
      .reduce((a, b) => a + b.recebido, 0);
    const divergente = concFiltradas
      .filter((c) => c.status === "Divergente" || c.status === "Parcial")
      .reduce((a, b) => a + Math.abs(b.esperado - b.recebido), 0);
    const pendente = concFiltradas
      .filter((c) => c.status === "Pendente")
      .reduce((a, b) => a + b.esperado, 0);
  ...
  const totalAtraso = cobrancasFiltradas
    .filter((c) => c.status === "Atrasado")
    .reduce((a, b) => a + b.valor, 0);
  const totalRecebido = cobrancasFiltradas
    .filter((c) => c.status === "Pago")
    .reduce((a, b) => a + b.valor, 0);
  const totalPendente = cobrancasFiltradas
    .filter((c) => c.status === "Pendente" || c.status === "Faturado")
    .reduce((a, b) => a + b.valor, 0);

  // Saúde financeira
  const totalCount = cobrancasFiltradas.length || 1;
  const inadimplentes = cobrancasFiltradas.filter(
    (c) => c.status === "Atrasado" || c.status === "Contestado",
  ).length;
  ...
  const margemTone: "green" | "amber" | "red" =
    margem < 0 ? "red" : margem < 20 ? "amber" : "green";

  // KPIs da aba Despesas
  const despPagas = despesasFiltradas
    .filter((d) => d.status === "Pago")
    .reduce((a, b) => a + b.valor, 0);
  const despAPagar = despesasFiltradas
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Cobranças, detalhamento de vendas e conciliação.
          </p>
        </div>
        <ExportarMenu
      ...
      {/* Resultado do período (Receita × Despesas × Margem) */}
      {(resultadoLiquido < 0 || totalDespesas / (receitaTotal || 1) > 0.7) && (
        <div className="space-y-2">
          {resultadoLiquido < 0 && (
            <div className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-800">
              ...
              <div>
                <span className="font-semibold">Resultado negativo no período.</span> Despesas
                excedem a receita em {formatBRL(Math.abs(resultadoLiquido))}.
              </div>
            </div>
          )}
          {resultadoLiquido >= 0 && totalDespesas / (receitaTotal || 1) > 0.7 && (
            <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              ...
              <div>
                <span className="font-semibold">Despesas elevadas.</span> Representam{" "}
                {((totalDespesas / (receitaTotal || 1)) * 100).toFixed(0)}% da receita do período.
              </div>
            </div>
          )}
        </div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mt-2 num font-display text-2xl text-emerald-700">
            {formatBRL(receitaTotal)}
          </div>
          <div className="mt-1 text-[11px] text-muted-foreground">Recebido + a receber</div>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
          ...
          <div className="mt-2 num font-display text-2xl">{formatBRL(totalDespesas)}</div>
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div
            className={cn(
              "mt-2 num font-display text-2xl",
              resultadoLiquido >= 0 ? "text-emerald-700" : "text-red-700",
            )}
          >
            {formatBRL(resultadoLiquido)}
          </div>
          <div className="mt-1 text-[11px] text-muted-foreground">Receita − despesas</div>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
            <span>Margem</span>
            <span
              className={cn(
                ...
                margemTone === "green" && "bg-emerald-500",
                margemTone === "amber" && "bg-amber-500",
                margemTone === "red" && "bg-red-500",
              )}
            />
          </div>
          <div
            className={cn(
              "mt-2 num font-display text-2xl",
              margemTone === "green" && "text-emerald-700",
              margemTone === "amber" && "text-amber-700",
              margemTone === "red" && "text-red-700",
            )}
          >
            {margem.toFixed(1)}%
          </div>
      ...
      <section className="grid gap-3 md:grid-cols-4">
        <MiniKPI
          label="Inadimplência"
          value={`${taxaInad.toFixed(1)}%`}
          hint={`${inadimplentes} de ${totalCount} cobranças`}
          tone={taxaInad > 25 ? "red" : taxaInad > 10 ? "amber" : "green"}
        />
        <MiniKPI
          label="Ticket médio"
          value={formatBRL(ticketMedio)}
          hint="Por cobrança no período"
        />
        <MiniKPI
          label="Tempo médio de pagamento"
          value={`${tempoMedio} dias`}
          hint="Da emissão até o pagamento"
        />
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mt-3 flex h-2 overflow-hidden rounded-full bg-surface">
            {receitaPorOrigem.map((r, i) => (
              <div
                key={r.origem}
                className={cn(i === 0 && "bg-navy", i === 1 && "bg-blue-500", i === 2 && "bg-warm")}
          ...
          <div className="mt-2 grid grid-cols-3 gap-1 text-[10px] text-muted-foreground">
            {receitaPorOrigem.map((r, i) => (
              <div key={r.origem} className="flex items-center gap-1">
                ...
                />
                <span>
                  {r.origem.split(" ")[0]} {Math.round((r.total / receitaTotalOrigem) * 100)}%
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="flex items-center gap-1 border-b border-border">
        <TabBtn active={tab === "cobrancas"} onClick={() => setTab("cobrancas")}>
          Cobranças
        </TabBtn>
        <TabBtn active={tab === "vendas"} onClick={() => setTab("vendas")}>
          Detalhamento de vendas
        </TabBtn>
        <TabBtn active={tab === "conciliacao"} onClick={() => setTab("conciliacao")}>
          Conciliação
        </TabBtn>
        <TabBtn active={tab === "despesas"} onClick={() => setTab("despesas")}>
          Despesas
        </TabBtn>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="mt-3 grid gap-3 border-t border-border pt-3 sm:grid-cols-4">
                ...
                <label className="flex items-end gap-2 text-xs">
                  <input
                    type="checkbox"
                    checked={apenasAlto}
                    onChange={(e) => setApenasAlto(e.target.checked)}
            ...
            <div className="mt-3 text-[11px] text-muted-foreground">
              Mostrando{" "}
              <span className="font-medium text-foreground">{cobrancasFiltradas.length}</span> de{" "}
              {cobrancasMock.length} cobranças
            </div>
          </div>
                    ...
                    return (
                      <tr key={c.id} className="hover:bg-surface/60">
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            {c.divergencia ? (
                              <Tooltip>
                                ...
                                <TooltipContent>
                                  Esperado {formatBRL(c.divergencia.esperado)} · Cobrado{" "}
                                  {formatBRL(c.divergencia.cobrado)} · Δ{" "}
                                  {formatBRL(c.divergencia.cobrado - c.divergencia.esperado)}
                                </TooltipContent>
                              </Tooltip>
                            ) : (
                              <Tooltip>
                                ...
                                <TooltipContent>
                                  Validado · origem confere com o valor
                                </TooltipContent>
                              </Tooltip>
                            )}
                        ...
                        <td className="px-4 py-3 text-muted-foreground">
                          {c.vencimento}
                          {c.diasAtraso ? (
                            <span className="ml-1 text-[10px] text-red-700">+{c.diasAtraso}d</span>
                          ) : null}
                        </td>
                          ...
                          <div className="flex justify-end gap-1">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                ...
                                <DropdownMenuItem
                                  onClick={() => toast.info(`Cobrança ${c.id} gerada`)}
                                >
                                  <Receipt className="mr-2 h-4 w-4" /> Gerar cobrança
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() => toast.warning(`${c.id} marcada como contestada`)}
                                >
                                ...
                                <DropdownMenuItem
                                  onClick={() => toast.info("Editor de cobrança em breve")}
                                >
            ...
            <div key={v.id} className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <div className="num font-display text-lg text-emerald-700">
                    {formatBRL(v.comissaoTotal)}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                  ...
                  <div className="num font-display text-lg">
                    {((v.comissaoTotal / v.valor) * 100).toFixed(1)}%
                  </div>
                </div>
              </div>
              <div className="mt-5">
                    ...
                    return (
                      <div key={i}>
                        <div className="flex items-center justify-between text-xs">
                          <span>
                            {s.nome} <span className="text-muted-foreground">· {s.tipo}</span>
                          </span>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <MiniKPI
              ...
              value={formatBRL(concKpis.conciliado)}
              tone="green"
              hint={`${concFiltradas.filter((c) => c.status === "Confirmada").length} venda(s)`}
            />
            <MiniKPI
              label="Total divergente"
              value={formatBRL(concKpis.divergente)}
              tone="red"
              hint={`${concFiltradas.filter((c) => c.status === "Divergente" || c.status === "Parcial").length} caso(s)`}
            />
            <MiniKPI
              label="Total pendente"
              value={formatBRL(concKpis.pendente)}
              tone="amber"
              hint={`${concFiltradas.filter((c) => c.status === "Pendente").length} aberto(s)`}
            />
            <MiniKPI
              label="Valor em risco"
              value={formatBRL(concKpis.risco)}
              tone="red"
              hint="Divergências + pendentes"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card p-3">
            ...
            <Select
              value={concStatusFiltro}
              onValueChange={(v) => setConcStatusFiltro(v as typeof concStatusFiltro)}
            >
              <SelectTrigger className="h-8 w-[150px] text-xs">
                <SelectValue placeholder="Status" />
              </SelectTrigger>
              <SelectContent>
                {(["Todos", "Confirmada", "Parcial", "Divergente", "Pendente"] as const).map(
                  (s) => (
            ...
            <Select
              value={concRiscoFiltro}
              onValueChange={(v) => setConcRiscoFiltro(v as typeof concRiscoFiltro)}
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="baixo" className="text-xs">
                  Baixo
                </SelectItem>
                <SelectItem value="medio" className="text-xs">
                  Médio
                </SelectItem>
                <SelectItem value="alto" className="text-xs">
                  Alto
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            <Input
              ...
              value={concValMin}
              onChange={(e) => setConcValMin(e.target.value)}
            />
            <Input
              className="h-8 w-[110px] text-xs"
              placeholder="Valor máx"
              value={concValMax}
              onChange={(e) => setConcValMax(e.target.value)}
            />
                ...
                <tbody className="divide-y divide-border">
                  {concAgrupado.map((g) => (
                    <tr key={g.corretor} className="hover:bg-surface/40">
                      ...
                      <td className="px-4 py-3 text-right num text-emerald-700">
                        {formatBRL(g.totalRecebido)}
                      </td>
                      <td className="px-4 py-3 text-right num text-amber-700">
                        {formatBRL(g.totalAtraso)}
                      </td>
                      <td
                        className={cn(
                          "px-4 py-3 text-right num font-medium",
                          g.inadimplenciaPct > 25
                            ? "text-red-700"
                            : g.inadimplenciaPct > 10
                              ? "text-amber-700"
                              : "text-emerald-700",
                        )}
                      >
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button
                          ...
                          onClick={() => {
                            setConcCorretor(g.corretor);
                            setConcAgrupar(false);
                          }}
                        >
                          Ver itens
                        </Button>
                    ...
                    return (
                      <tr
                        ...
                        className={cn(
                          ...
                          (c.status === "Divergente" || c.status === "Parcial") && "bg-red-50/40",
                        )}
                        onClick={() => setConcDetalhe(c)}
                      >
                        <td className="px-4 py-3 font-mono text-xs text-muted-foreground">
                          <div className="flex items-center gap-1.5">
                            {alertas.map((a, i) => (
                              <TooltipProvider key={i}>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <span className="cursor-help">{a.icon}</span>
                                  </TooltipTrigger>
                                  <TooltipContent>
                            ...
                            {bloqueada && (
                              <TooltipProvider>
                                <Tooltip>
                                  ...
                                  <TooltipContent>
                                    <p className="text-xs">Conciliação confirmada — bloqueada</p>
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                            )}
                          ...
                          <HoverCard>
                            ...
                            <HoverCardContent className="w-72 text-xs">
                              ...
                              <ul className="space-y-1 text-muted-foreground">
                                <li>
                                  Dias desde fatura:{" "}
                                  <span className="text-foreground">{c.diasDesdeFatura}</span>
                                </li>
                                <li>
                                  Divergência:{" "}
                                  <span className="text-foreground">
                                    {c.esperado === 0
                                      ? "0%"
                                      : `${(((c.esperado - c.recebido) / c.esperado) * 100).toFixed(1)}%`}
                                  </span>
                                </li>
                                <li>
                                  Histórico do corretor:{" "}
                                  <span className="text-foreground">
                                    {corretorRisco[c.corretor]?.nivel ?? "n/d"} ·{" "}
                                    {corretorRisco[c.corretor]?.pctAtraso ?? 0}% atraso
                                  </span>
                                </li>
                              </ul>
                            </HoverCardContent>
                            ...
                            onAtribuir={(r) =>
                              atualizarConciliacao(
                                ...
                                { responsavel: r },
                                {
                                  ...
                                  autor: "Superadmin",
                                  acao: c.responsavel
                                    ? `Responsável reatribuído para ${r.nome}`
                                    : `Responsável atribuído: ${r.nome}`,
                                },
                              )
                            }
                          />
                        ...
                        <td className="px-4 py-3 text-xs">
                          {bloqueada ? (
                            ...
                            >
                              {sla.atrasado
                                ? `Atrasado +${Math.abs(sla.restanteDias)}d`
                                : `Resolver em ${sla.restanteDias}d`}
                            </span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-xs" onClick={(e) => e.stopPropagation()}>
                            ...
                            onSet={(d) =>
                              atualizarConciliacao(
                                ...
                                { previsaoPagamento: d },
                                {
                                  data: agora(),
                                  autor: "Superadmin",
                                  acao: `Previsão de pagamento definida: ${d}`,
                              ...
                              {bloqueada ? (
                                <>
                                  ...
                                  <DropdownMenuItem
                                    onClick={() => {
                                      setConcReabrir(c);
                                      setConcReabrirJustificativa("");
                                    }}
                                  >
                                    <RotateCcw className="mr-2 h-4 w-4 text-amber-600" /> Reabrir
                                    conciliação
                                  </DropdownMenuItem>
                                </>
                              ) : (
                                <>
                                  ...
                                  <DropdownMenuItem
                                    onClick={() => {
                                      atualizarConciliacao(
                                        c.id,
                                        { recebido: c.esperado, pagoEm: hojeStr() },
                                        {
                                          data: agora(),
                                          autor: "Superadmin",
                                          acao: "Pagamento confirmado",
                                          valorAnterior: c.recebido,
                                          valorNovo: c.esperado,
                                        },
                                      );
                                      toast.success(`Pagamento de ${c.id} confirmado`);
                                    }}
                                  >
                                    ...
                                    onClick={() => {
                                      atualizarConciliacao(
                                        ...
                                        {},
                                        {
                                          data: agora(),
                                          autor: "Superadmin",
                                          acao: "Marcado como divergente manualmente",
                                        },
                                      );
                                      toast.warning(`${c.id} marcado para revisão`);
                                    }}
                                  >
                                    <AlertTriangle className="mr-2 h-4 w-4 text-amber-600" /> Marcar
                                    como divergente
                                  </DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => setConcDetalhe(c)}>
                                  ...
                                  <DropdownMenuItem
                                    onClick={() =>
                                      toast.info(`Contrato da parceria ${c.venda}`, {
                                        description: "Documento abriria em nova aba (mock).",
                                      })
                                    }
                                  >
                                    <FileSignature className="mr-2 h-4 w-4" /> Ver contrato da
                                    parceria
                                  </DropdownMenuItem>
                                </>
                              )}
                  ...
                  {concFiltradas.length === 0 && (
                    <tr>
                      <td
                        ...
                        className="px-4 py-12 text-center text-sm text-muted-foreground"
                      >
                        Nenhuma conciliação para os filtros aplicados.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          >
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Esta ação reabre a conciliação para edição. A justificativa será registrada no log
                  de auditoria.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <Textarea
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  onClick={() => {
                    if (concReabrirJustificativa.trim().length < 10) {
                      toast.error("Justificativa obrigatória (mín. 10 caracteres)");
                      return;
                    }
                    if (concReabrir)
                      reabrirConciliacao(concReabrir, concReabrirJustificativa.trim());
                    setConcReabrir(null);
                    setConcReabrirJustificativa("");
                  }}
                >
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <MiniKPI
              ...
              value={formatBRL(despesasFiltradas.reduce((a, b) => a + b.valor, 0))}
              hint={`${despesasFiltradas.length} lançamento(s)`}
            />
            <MiniKPI
              label="Pagas"
              value={formatBRL(despPagas)}
              tone="green"
              hint={`${despesasFiltradas.filter((d) => d.status === "Pago").length} item(ns)`}
            />
            <MiniKPI
              label="A pagar"
              value={formatBRL(despAPagar)}
              tone="amber"
              hint={`${despesasFiltradas.filter((d) => d.status === "A pagar").length} pendente(s)`}
            />
            <MiniKPI
              label="Maior categoria"
              value={maiorCategoria ? maiorCategoria[0] : "—"}
              hint={maiorCategoria ? formatBRL(maiorCategoria[1]) : "Sem despesas"}
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card p-3">
            ...
            <Select
              value={despFiltroCat}
              onValueChange={(v) => setDespFiltroCat(v as typeof despFiltroCat)}
            >
            ...
            <Select
              value={despFiltroStatus}
              onValueChange={(v) => setDespFiltroStatus(v as typeof despFiltroStatus)}
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="A pagar" className="text-xs">
                  A pagar
                </SelectItem>
              </SelectContent>
            </Select>
            <Select
              value={despFiltroTipo}
              onValueChange={(v) => setDespFiltroTipo(v as typeof despFiltroTipo)}
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="Fixo" className="text-xs">
                  Fixo
                </SelectItem>
                <SelectItem value="Variável" className="text-xs">
                  Variável
                </SelectItem>
              </SelectContent>
            </Select>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          ...
                          {d.status === "A pagar" && (
                            <DropdownMenuItem
                              onClick={() => {
                                setDespesas((arr) =>
                                  arr.map((x) => (x.id === d.id ? { ...x, status: "Pago" } : x)),
                                );
                                toast.success("Despesa marcada como paga", {
                                  description: `${d.id} · ${formatBRL(d.valor)}`,
                                });
                              }}
                            >
      ...
      <DespesaModal
        ...
        onSave={(d) => {
          ...
          toast.success(despesaEdit ? "Despesa atualizada" : "Despesa criada", {
            description: `${d.descricao} · ${formatBRL(d.valor)}`,
          });
          setDespesaModalOpen(false);
          setDespesaEdit(null);
        }}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação remove o lançamento e recalcula o resultado do período. Não pode ser
              desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function ExportarMenu({
  ...
  const exportar = (
    ...
    if (tipo === "filtrados") {
      ...
      rows = Object.entries(grupos).map(([k, v]) => ({
        Corretor: k,
        Cobranças: v.qtd,
        Total: v.total,
        EmAberto: v.aberto,
      ...
      rows = [
        { Categoria: "SaaS (recorrente)", Valor: saas },
        { Categoria: "Comissão (parceria/lead)", Valor: com },
        { Categoria: "Total", Valor: saas + com },
      ];
      nome = "relatorio-contabil";
    } else if (tipo === "inadimplencia") {
      rows = cobrancas
        .filter((c) => c.status === "Atrasado" || c.status === "Contestado")
        .map((c) => ({
          ID: c.id,
          Corretor: c.corretor,
          Valor: c.valor,
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {despesa
              ? `Atualize os dados da despesa ${despesa.id}.`
              : "Lançamentos impactam automaticamente o resultado do período."}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1 sm:col-span-2">
            ...
            <Input
              ...
              placeholder="Ex.: Mídia paga — Meta Ads"
            />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
...
function CobrancaDetalheModal({
  ...
  const venda = cobranca?.vendaId
    ? vendasDetalhadas.find((v) => v.id === cobranca.vendaId)
    : undefined;
  return (
        ...
        {cobranca && (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                {cobranca.corretor} · {cobranca.origem} · {formatBRL(cobranca.valor)}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-5">
                ...
                {venda ? (
                  <div className="grid gap-2 text-sm sm:grid-cols-3">
                    <div>
                      ...
                      <div>{venda.imovel}</div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                        VGV
                      </div>
                      <div className="num">{formatBRL(venda.valor)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                        Tipo
                      </div>
                      <div>{cobranca.origem}</div>
                    </div>
                  </div>
                ) : (
                  <div className="text-sm text-muted-foreground">
                    Cobrança{" "}
                    {cobranca.origem === "SaaS"
                      ? "SaaS recorrente — sem venda associada"
                      : "sem venda associada"}
                    .
                  </div>
                )}
                ...
                <Section title="Comissão">
                  <div className="mb-3 grid gap-2 text-sm sm:grid-cols-3">
                    <div>
                      ...
                      <div className="num">
                        {((venda.comissaoTotal / venda.valor) * 100).toFixed(2)}%
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                      ...
                      <div className="num">{formatBRL(venda.comissaoTotal)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                        Valor cobrado
                      ...
                      <div
                        className={cn("num", cobranca.divergencia && "text-amber-700 font-medium")}
                      >
                  ...
                  {cobranca.divergencia && (
                    <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                      ⚠ Divergência detectada — Δ{" "}
                      {formatBRL(cobranca.divergencia.cobrado - cobranca.divergencia.esperado)}
                    </div>
                  )}
              ...
              <Section title="Envolvidos">
                <div className="flex flex-wrap gap-2">
                  {(venda
                    ? venda.splits.filter((s) => s.tipo !== "Fee Ubroker").map((s) => s.nome)
                    : [cobranca.corretor]
                  ).map((n) => (
                    <span
                      key={n}
                    ...
                    >
                      <span className="grid h-5 w-5 place-items-center rounded-full bg-navy text-[9px] font-semibold text-white">
                        {n
                          .split(" ")
                          .map((p) => p[0])
                          .slice(0, 2)
              ...
              <Section title="Timeline financeira">
                <ol className="space-y-3">
                  <Step
                    ...
                    label="Criada em"
                    date={cobranca.criadoEm}
                  />
                  <Step
                    icon={<Receipt className="h-3.5 w-3.5" />}
                    label="Faturada em"
                    date={cobranca.faturadoEm ?? "—"}
                  />
                  <Step
                    ...
                    extra={
                      cobranca.diasAtraso ? (
                        <span className="rounded-full bg-red-100 px-1.5 py-0.5 text-[10px] font-medium text-red-700">
                          {cobranca.diasAtraso} dias em atraso
                        </span>
                      ) : null
                    }
                  />
                  <Step
                    ...
                    label="Pago em"
                    date={cobranca.pagoEm ?? "—"}
                    done={!!cobranca.pagoEm}
                  />
                </ol>
              </Section>
            </div>
    ...
    if (tipo === "divergencias") {
      rows = conciliacoes
        ...
        .map((c) => ({
          ...
          Esperado: c.esperado,
          Recebido: c.recebido,
          Diferenca: c.esperado - c.recebido,
          Status: c.status,
        }));
      nome = "relatorio-divergencias";
    } else if (tipo === "inadimplencia") {
      rows = conciliacoes
        .filter((c) => c.status === "Pendente" || (c.recebido === 0 && c.diasDesdeFatura > 15))
        .map((c) => ({
          ID: c.id,
          Corretor: c.corretor,
          Esperado: c.esperado,
          DiasDesdeFatura: c.diasDesdeFatura,
      ...
      const grupos: Record<
        string,
        { qtd: number; esperado: number; recebido: number; diferenca: number }
      > = {};
      conciliacoes.forEach((c) => {
        const g = (grupos[c.corretor] ||= { qtd: 0, esperado: 0, recebido: 0, diferenca: 0 });
        g.qtd += 1;
        g.esperado += c.esperado;
        g.recebido += c.recebido;
        g.diferenca += c.esperado - c.recebido;
      });
      rows = Object.entries(grupos).map(([k, v]) => ({
        Corretor: k,
        Casos: v.qtd,
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-64">
        ...
        <DropdownMenuItem onClick={() => exportar("divergencias")}>
          <FileText className="mr-2 h-4 w-4" /> Divergências
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => exportar("inadimplencia")}>
          <FileText className="mr-2 h-4 w-4" /> Inadimplência
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => exportar("corretor")}>
          <FileText className="mr-2 h-4 w-4" /> Por corretor
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => exportar("auditoria")}>
          <FileText className="mr-2 h-4 w-4" /> Auditoria completa
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
function ConciliacaoDetalheModal({
  ...
  const handleRegistrarDivergencia = () => {
    onUpdate(
      c.id,
      {},
      { data: agora(), autor: "Superadmin", acao: "Divergência registrada manualmente" },
    );
  ...
  const handleRegistrarCobranca = () => {
    onUpdate(
      c.id,
      {},
      { data: agora(), autor: "Superadmin", acao: "Cobrança realizada registrada" },
    );
            ...
            <Section title="1 · Resumo da venda">
              <div className="grid gap-2 text-sm sm:grid-cols-3">
                ...
                <div>
                  ...
                  <div className="num">{formatBRL(c.vgv)}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                    Tipo
            ...
            <Section title="2 · Comissão detalhada">
              <div className="mb-3 grid gap-2 text-sm sm:grid-cols-3">
                <div>
                  ...
                  <div className="num">{c.comissaoPct.toFixed(2)}%</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                    Comissão total
                  </div>
                  <div className="num">{formatBRL(c.comissaoTotal)}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                    Fee Ubroker
                  </div>
                  <div className="num">{formatBRL(c.esperado)}</div>
                </div>
              </div>
            ...
            <Section title="3 · Conciliação">
              <div className="grid gap-2 text-sm sm:grid-cols-4">
                ...
                <div>
                  ...
                  <div className="num">{formatBRL(c.recebido)}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                    Diferença
              ...
              {alertasConc(c).length > 0 && (
                <div className="mt-3 space-y-1.5">
                  {alertasConc(c).map((a, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-1.5 text-xs text-amber-800"
                    >
                      {a.icon} {a.msg}
                    </div>
                  ))}
            ...
            <Section title="4 · Ações diretas">
              {bloqueada && (
                <div className="mb-3 flex items-center gap-2 rounded-md border border-emerald-300 bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
                  <Lock className="h-3.5 w-3.5" /> Conciliação confirmada — edição bloqueada. Use
                  "Reabrir conciliação" no menu da linha para alterar valores.
                </div>
              )}
              <div className="flex flex-wrap gap-2">
                <Button
                ...
                <Button
                  ...
                  disabled={bloqueada}
                  onClick={() => {
                    setNovoValor(String(c.recebido));
                    setAjustarOpen(true);
                  }}
                >
                  <Pencil className="h-4 w-4" /> Ajustar valor
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                ...
                >
                  <AlertTriangle className="h-4 w-4 text-amber-600" /> Registrar divergência
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                ...
                >
                  <Phone className="h-4 w-4" /> Registrar cobrança realizada
                </Button>
              </div>
              <div className="mt-3 flex items-center gap-2 text-xs">
                <span className="text-muted-foreground">Status operacional:</span>
                <Select
                  value={opSel === "—" ? c.statusOperacional : opSel}
                  onValueChange={(v) => {
                    const s = v as StatusOperacionalCobranca;
                    setOpSel(s);
                    onUpdate(
                      c.id,
                      { statusOperacional: s },
                      { data: agora(), autor: "Superadmin", acao: `Status operacional → ${s}` },
                    );
                  }}
                  disabled={bloqueada}
                >
              ...
              <ol className="space-y-2.5">
                {c.auditoria.map((a, i) => (
                  <li key={i} className="flex items-start gap-3 text-xs">
                    <span className="mt-0.5 grid h-6 w-6 place-items-center rounded-full border border-border bg-card text-muted-foreground">
                    ...
                    {bloqueada ? (
                      ...
                      <div className="space-y-1">
                        <div
                          className={cn(
                            "text-xs font-medium",
                            sla.atrasado ? "text-red-700" : "text-foreground",
                          )}
                        >
                          {sla.atrasado
                            ? `Atrasado +${Math.abs(sla.restanteDias)}d`
                        ...
                        <div className="h-1.5 w-full overflow-hidden rounded-full bg-surface">
                          <div
                            className={cn("h-full", sla.atrasado ? "bg-red-500" : "bg-emerald-500")}
                            style={{
                              width: `${sla.atrasado ? 100 : Math.max(10, ((c.slaDias - Math.max(sla.restanteDias, 0)) / c.slaDias) * 100)}%`,
                            }}
                          />
                        </div>
                      </div>
              ...
              <div className="grid gap-3 sm:grid-cols-4 text-sm">
                <div>
                  ...
                  <div
                    className={cn(
                      "num font-medium",
                      c.historicoCorretor.pagamentosAtrasoPct > 25
                        ? "text-red-700"
                        : c.historicoCorretor.pagamentosAtrasoPct > 10
                          ? "text-amber-700"
                          : "text-emerald-700",
                    )}
                  >
                    {c.historicoCorretor.pagamentosAtrasoPct}%
                  </div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                  ...
                  <div className="num">{c.historicoCorretor.tempoMedioPagamentoDias}d</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                    Total já pago ao hub
                  </div>
                  <div className="num text-emerald-700">
                    {formatBRL(c.historicoCorretor.totalPagoHub)}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                  ...
                  <div
                    className={cn(
                      "num",
                      c.historicoCorretor.totalAberto > 0
                        ? "text-amber-700"
                        : "text-muted-foreground",
                    )}
                  >
                    {formatBRL(c.historicoCorretor.totalAberto)}
                  </div>
                </div>
              </div>
            </Section>
            ...
            <Section title="8 · Contrato aplicado">
              <div className="grid gap-2 text-sm sm:grid-cols-3">
                <div>
                  ...
                  <div className="font-medium">{c.contrato.versao}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                    Vigência
                  </div>
                  <div>{c.contrato.data}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                    Regras de comissão
                  </div>
                  <div className="text-xs text-muted-foreground">{c.contrato.regras}</div>
                </div>
              </div>
              <Button
                size="sm"
                ...
                onClick={() =>
                  toast.info(`Abrir contrato ${c.contrato.versao}`, {
                    description: "Documento abriria em nova aba (mock).",
                  })
                }
              >
            ...
            <Section title="CRM de cobrança · Interações">
              {c.interacoes.length > 0 ? (
                <ul className="mb-3 space-y-2">
                  {c.interacoes.map((it, i) => (
                    ...
                    >
                      <span className="mt-0.5">{iconForInteracao(it.tipo)}</span>
                      <div className="flex-1">
              ...
              <div className="flex flex-wrap items-end gap-2">
                <div className="flex-1 min-w-[140px]">
                  ...
                  <Select
                    value={intTipo}
                    onValueChange={(v) => setIntTipo(v as ConciliacaoInteracao["tipo"])}
                  >
                    <SelectTrigger className="h-8 text-xs">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="Ligação" className="text-xs">
                        Ligação
                      </SelectItem>
                      <SelectItem value="WhatsApp" className="text-xs">
                        WhatsApp
                      </SelectItem>
                      <SelectItem value="E-mail" className="text-xs">
                        E-mail
                      </SelectItem>
                      <SelectItem value="Mensagem" className="text-xs">
                        Mensagem
                      </SelectItem>
                      <SelectItem value="Negociação" className="text-xs">
                        Negociação
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <Button size="sm" onClick={handleInteracao}>
                  Registrar
                </Button>
              </div>
            </Section>
      ...
      <AlertDialog open={confirmarOpen} onOpenChange={setConfirmarOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {c.id} · {c.corretor} — registrar recebimento de {formatBRL(c.esperado)}. Esta ação
      ...
      <AlertDialog open={ajustarOpen} onOpenChange={setAjustarOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {c.id} · {c.corretor}. Valor anterior: {formatBRL(c.recebido)}. Esperado:{" "}
              {formatBRL(c.esperado)}.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <Input
            type="number"
            ...
            placeholder="Novo valor recebido (R$)"
          />
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
...
function ResponsavelChip({
  ...
  c: Conciliacao;
  onAtribuir: (r: ResponsavelCobranca) => void;
}) {
  return (
    <Popover>
      <PopoverTrigger asChild>
        {c.responsavel ? (
          <button className="inline-flex items-center gap-1.5 rounded-full bg-surface px-2 py-0.5 text-[11px] hover:bg-surface/70">
            <span className="grid h-4 w-4 place-items-center rounded-full bg-navy text-[8px] font-semibold text-white">
              {c.responsavel.nome
                .split(" ")
                .map((p) => p[0])
                .slice(0, 2)
      ...
      <PopoverContent align="start" className="w-56 p-2 text-xs">
        <div className="mb-1.5 text-[10px] uppercase tracking-widest text-muted-foreground">
          {c.responsavel ? "Reatribuir" : "Atribuir responsável"}
        </div>
        <div className="space-y-1">
          {RESPONSAVEIS_DISPONIVEIS.map((r) => (
            <button
              ...
              onClick={() => {
                onAtribuir(r);
                toast.success(`Responsável: ${r.nome}`);
              }}
              className={cn(
...
function ComprovanteBlock({
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const comp: ComprovantePagamento = {
      nome: f.name,
      tipo,
    ...
    onUpdate(
      c.id,
      { comprovante: comp },
      { data: agora(), autor: "Superadmin", acao: `Comprovante anexado: ${f.name}` },
    );
  ...
  const handleRemover = () => {
    onUpdate(
      ...
      { data: agora(), autor: "Superadmin", acao: `Comprovante removido` },
    );
    ...
    <div className="space-y-3">
        ...
        <div className="flex items-center justify-between rounded-md border border-border bg-card px-3 py-2 text-xs">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-muted-foreground">
                {c.comprovante.tipo} · {c.comprovante.referencia ?? "—"} · {c.comprovante.enviadoEm}
              </div>
            </div>
          </div>
          {!bloqueada && (
      ...
      <div className="flex flex-wrap items-end gap-2">
        <div className="flex-1 min-w-[180px]">
          <div className="mb-1 text-[10px] uppercase tracking-widest text-muted-foreground">
            Referência (PIX/TED/banco)
          </div>
          <Input
            value={referencia}
            onChange={(e) => setReferencia(e.target.value)}
            placeholder="Ex: PIX BB · E2E…"
            className="h-8 text-xs"
            disabled={bloqueada}
          />
        </div>
        <label
          className={cn(
            "inline-flex items-center gap-2 rounded-md border border-border bg-card px-3 py-1.5 text-xs",
            bloqueada ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:bg-surface",
          )}
        >
