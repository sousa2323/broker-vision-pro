    ...
    <div className="max-w-3xl space-y-6">
      ...
      <Section icon={Bell} title="Notificações" desc="Como queremos te avisar.">
        ...
        <SubGroup label="Leads">
          <Toggle label="Novo lead por e-mail" v={s.leadsEmail} onChange={() => toggle("leadsEmail")} />
          <Toggle label="Novo lead via push (mobile)" v={s.leadsPush} onChange={() => toggle("leadsPush")} />
        </SubGroup>

        <SubGroup label="Parcerias">
          <Toggle label="Convites de parceria" v={s.parceriasConvites} onChange={() => toggle("parceriasConvites")} />
          <Toggle label="Atualizações de proposta" v={s.propostasUpdates} onChange={() => toggle("propostasUpdates")} />
          <Toggle label="Parceria fechada" v={s.parceriaFechada} onChange={() => toggle("parceriaFechada")} />
        </SubGroup>

        <SubGroup label="Performance">
          <Toggle label="Resumo diário (18h)" v={s.digest} onChange={() => toggle("digest")} />
          <Toggle label="Insights semanais da IA" v={s.insightsIA} onChange={() => toggle("insightsIA")} />
        </SubGroup>
      </Section>

      {/* AUTOMAÇÕES */}
      <Section icon={Zap} title="Automação de processos" desc="Deixe o sistema trabalhar por você.">
        <Toggle label="Criar atividade automática ao receber lead" v={s.autoAtividade} onChange={() => toggle("autoAtividade")} />
        <Toggle label="Criar follow-up automático após 24h sem resposta" v={s.autoFollowup} onChange={() => toggle("autoFollowup")} />
        <Toggle label="Sugerir imóveis automaticamente via IA" v={s.autoSugestoes} onChange={() => toggle("autoSugestoes")} />
        <Toggle label="Lembrar de atualizar etapa do pipeline" v={s.autoLembretePipeline} onChange={() => toggle("autoLembretePipeline")} />
      </Section>

      {/* PRIVACIDADE */}
      <Section icon={Eye} title="Privacidade e visibilidade" desc="Controle sua participação no ecossistema.">
        <Toggle label="Tornar meu perfil visível para outros corretores" v={s.perfilVisivel} onChange={() => toggle("perfilVisivel")} />
        <Toggle label="Permitir receber solicitações de parceria" v={s.aceitaParcerias} onChange={() => toggle("aceitaParcerias")} />
        <Toggle label="Exibir meus imóveis para a rede" v={s.exibeImoveisRede} onChange={() => toggle("exibeImoveisRede")} />
      </Section>
      ...
      {/* PLANO E COBRANÇA */}
      <Section icon={CreditCard} title="Plano e cobrança" desc="Gerencie seu plano e recursos ativos.">
        <Pref label="Plano atual" value="Free" />
        <Pref label="Valor mensal" value="R$ 0/mês" />
      ...
      <Section icon={Smartphone} title="Integrações" desc="Conecte seus canais.">
        <Integracao nome="WhatsApp Business" sub="+55 21 99812-4477" conectado />
...
function Integracao({
  nome, sub, conectado,
}: { nome: string; sub: string; conectado?: boolean }) {
  return (
