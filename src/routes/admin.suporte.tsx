  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Casos abertos entre corretores, contestações e bypass.</p>
      </div>

      <div className="flex items-center gap-1 border-b border-border">
        <TabBtn active={tab === "disputas"} onClick={() => setTab("disputas")}>Disputas ({disputas.length})</TabBtn>
        <TabBtn active={tab === "contestacoes"} onClick={() => setTab("contestacoes")}>Contestações de venda</TabBtn>
        <TabBtn active={tab === "bypass"} onClick={() => setTab("bypass")}>Possíveis bypass ({bypassAlertas.length})</TabBtn>
      </div>
          ...
          {disputas.map((d) => (
            <div key={d.id} className="rounded-xl border border-border bg-card p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-xs text-muted-foreground">{d.id} · {d.data}</div>
                  <div className="mt-1 font-display text-lg">{d.partes[0]} <span className="text-muted-foreground">vs</span> {d.partes[1]}</div>
                </div>
                <span className={cn(
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {[
                { v: "VD-117", q: "Aldemar Souza", m: "Discordância sobre split de comissão.", r: "R$ 18.900", s: "Em análise" as const },
                { v: "VD-114", q: "Pedro Verissimo", m: "Cliente alega comissão duplicada cobrada.", r: "R$ 6.400", s: "Aberta" as const },
                { v: "VD-110", q: "Beatriz Lemos", m: "Fee Ubroker contestado por divergência de regra.", r: "R$ 2.100", s: "Resolvida" as const },
              ].map((c, i) => (
        ...
        <div className="space-y-3">
          {bypassAlertas.map((b) => (
            <div key={b.id} className={cn(
              ...
              b.risco === "Alto" ? "border-red-200 bg-red-50/40" : "border-amber-200 bg-amber-50/40",
            )}>
              <div className="flex items-start gap-4">
                <span className={cn(
                  ...
                  {b.risco === "Alto" ? <ShieldAlert className="h-5 w-5" /> : <AlertTriangle className="h-5 w-5" />}
                </span>
                ...
                <button className="rounded-md bg-foreground px-3 py-1.5 text-xs text-background hover:opacity-90">Investigar</button>
              </div>
            </div>
          ))}
        </div>
